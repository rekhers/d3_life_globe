...
function queueData() {
  queue()
    .defer(
      d3.json,
      "https://raw.githubusercontent.com/rekhers/d3_maps/gh-pages/data/world-110m2.json"
    )
...
function ready(error, world, names, cities) {
  ...
  var amherst = {
    geometry: { coordinates: [-72.519854, 42.373222], type: "Point" },
    type: "Feature",
    properties: { city: "Amherst", wikipedia: "Amherst,_Massachusetts" },
    id: "Amherst"
  };
  ...
  $("#listDiv div").hover(function() {
    ...
    svg
      ...
      .datum({
        type: "Point",
        coordinates: [
          cityCoords[currCity].geometry.coordinates[0],
          cityCoords[currCity].geometry.coordinates[1]
        ]
      })
      .attr("class", "points")
      .attr("d", path)
    ...
    svg
      ...
      .datum({
        type: "LineString",
        coordinates: [[-97, 30], [-0.117, 51.5]] // points in decimal degrees
      })
      .attr("d", path)
    ...
    if (lastCity) {
      svg
        ...
        .datum({
          type: "Point",
          coordinates: [
            cityCoords[lastCity].geometry.coordinates[0],
            cityCoords[lastCity].geometry.coordinates[1]
          ]
        })
        .attr("class", "points")
        .attr("d", path)
